...
class HoudiniEngine(sgtk.platform.Engine):
    ...
    def post_app_init(self):
        ...
        if bootstrap.g_temp_env in os.environ:
            ...
            # resolve the issue.
            if self.has_ui:
                QtCore.QTimer.singleShot(100, _load_otls)
            else:
                _load_otls()
        ...
        if bootstrap.g_temp_env in os.environ:
            ...
            if commands and enable_sg_shelf:
                ...
                # shelves.
                #
                # April 2021: Looks like this now impacts more than just Windows.
                # Opening it up to all OSs.
                QtCore.QTimer.singleShot(
                    100,
                    _poll_for_ui_available_then_setup_shelves,
                )
